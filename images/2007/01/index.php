	...
	fclose($handle);
      ...
      if ( ! ( strpos($entry, ".3gp" ) === false ) ) {
        print "<a href=".$entry." type=video/3gpp>";
      	print "<embed src=".$entry." type=video/3gpp width=320 height=300>";
	print "</a><br>";
        print "<a href=".$entry." type=video/3gpp>";
...
$ithumb = 0;
print"<h2>Images (".count($dlist).")</h2><ul>\r\n";

if ( $HTTP_GET_VARS[all] != 'yes' && count($dlist) > 20 ) {
  print"Note: Above 20 images, only the first 10, last 10, and selected images in between are shown in thumbnail mode\r\n";
  print "you can see all the images by cicking on an image and pressing Next\r\n";
  print "<a href=index.php?all=yes>View all thumbnails</a>.\r\n";
  print "<hr>\r\n";
}

// Show the first 10, last 10, and every 10th in between
for ( $i = 0; $i < count($dlist); $i++ ) {
  if ( $HTTP_GET_VARS[all] != 'yes' && $i > 10 && $i < (count($dlist)-10) && ($i % 10 ) != 0 ) {
    continue;
  }
  $entry = $dlist[$i];
